...
int main(int argc, char *argv[]) {
    ...
    if (fd_cache_miss == -1) { perror("perf_event_open (cache misses)"); exit(1); }
          ...
          __typeof__(search_old_relocation(reloc_table,addr_to_patch,n_reloc)) __perf_ret = search_old_relocation(reloc_table,addr_to_patch,n_reloc);
    ...
    // Write data
    fprintf(f_csv, "search_old_relocation,%lu,%lu,%lu,%lu newline",
            count_cycles,
            count_instructions,
            count_cache_miss,
            count_cache_ref);
