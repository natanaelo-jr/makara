...
int main(int argc, char *argv[]) {
    ...
    if (fd_cache_miss == -1) { perror("perf_event_open (cache misses)"); exit(1); }
          ...
          __typeof__(stb_bitset_unioneq_changed(p0,p1,len)) __perf_ret = stb_bitset_unioneq_changed(p0,p1,len);
    ...
    // Write data
    fprintf(f_csv, "stb_bitset_unioneq_changed,%lu,%lu,%lu,%lu newline",
            count_cycles,
            count_instructions,
            count_cache_miss,
            count_cache_ref);
