...
typedef enum netxbig_led_mode {
  ____Placeholder_netxbig_led_mode
} netxbig_led_mode;
...
netxbig_led_get_timer_mode(enum netxbig_led_mode *mode, unsigned long delay_on,
                           unsigned long delay_off,
                           struct netxbig_led_timer *timer, int num_timer) {
  int i;

  for (i = 0; i < num_timer; i++) {
    if (timer[i].delay_on == delay_on && timer[i].delay_off == delay_off) {
      *mode = timer[i].mode;
      return 0;
    }
  }
...
int main(int argc, char *argv[]) {
  ...
  case 0: {
    ...
    enum netxbig_led_mode *mode = (enum netxbig_led_mode *)malloc(
        _len_mode0 * sizeof(enum netxbig_led_mode));
    for (int _i0 = 0; _i0 < _len_mode0; _i0++) {
      mode[_i0] = 0;
    }

    int _len_timer0 = 65025;
    struct netxbig_led_timer *timer = (struct netxbig_led_timer *)malloc(
        _len_timer0 * sizeof(struct netxbig_led_timer));
    ...
    int benchRet = ({
      ...
      __typeof__(netxbig_led_get_timer_mode(mode, delay_on, delay_off, timer,
                                            num_timer)) __perf_ret =
          netxbig_led_get_timer_mode(mode, delay_on, delay_off, timer,
                                     num_timer);
      ...
      // Write data
      fprintf(f_csv, "netxbig_led_get_timer_mode,%lu,%lu,%lu,%lu newline",
