...
int main(int argc, char *argv[]) {
    ...
    if (fd_cache_miss == -1) { perror("perf_event_open (cache misses)"); exit(1); }
          ...
          __typeof__(get_intersections(lines,intersections,n)) __perf_ret = get_intersections(lines,intersections,n);
    ...
    // Write data
    fprintf(f_csv, "get_intersections,%lu,%lu,%lu,%lu newline",
            count_cycles,
            count_instructions,
            count_cache_miss,
            count_cache_ref);
