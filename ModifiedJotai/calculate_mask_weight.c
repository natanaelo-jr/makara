...
__attribute__((used)) static unsigned calculate_mask_weight(uint32 mask) {
  unsigned weight = 0;
  do {
    weight += (mask & 1);
  } while (mask >>= 1);
...
int main(int argc, char *argv[]) {
  ...
  case 0: {
    ...
    unsigned int benchRet = ({
      ...
      __typeof__(calculate_mask_weight(mask)) __perf_ret =
          calculate_mask_weight(mask);
      ...
      // Write data
      fprintf(f_csv, "calculate_mask_weight,%lu,%lu,%lu,%lu newline",
