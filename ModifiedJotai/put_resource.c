...
__attribute__((used)) static int put_resource(u8 *rscs, unsigned int multi,
                                              unsigned int idx) {
...
int main(int argc, char *argv[]) {
  ...
  case 0: {
    ...
    int _len_rscs0 = 100;
    unsigned int *rscs =
        (unsigned int *)malloc(_len_rscs0 * sizeof(unsigned int));
    ...
    int benchRet = ({
      ...
      __typeof__(put_resource(rscs, multi, idx)) __perf_ret =
          put_resource(rscs, multi, idx);
      ...
      // Write data
      fprintf(f_csv, "put_resource,%lu,%lu,%lu,%lu newline", count_cycles,
