...
int main(int argc, char *argv[]) {
    ...
    if (fd_cache_miss == -1) { perror("perf_event_open (cache misses)"); exit(1); }
          ...
          __typeof__(netxbig_led_get_timer_mode(mode,delay_on,delay_off,timer,num_timer)) __perf_ret = netxbig_led_get_timer_mode(mode,delay_on,delay_off,timer,num_timer);
