...
int size_from_channelarray(struct iio_channel_info *channels,
  ...
  while (i < num_channels) {
    if (bytes % channels[i].bytes == 0)
      ...
      channels[i].location =
          bytes - bytes % channels[i].bytes + channels[i].bytes;
...
int main(int argc, char *argv[]) {
  ...
  case 0: {
    ...
    struct iio_channel_info *channels = (struct iio_channel_info *)malloc(
        _len_channels0 * sizeof(struct iio_channel_info));
    ...
    int benchRet = ({
      ...
      __typeof__(size_from_channelarray(channels, num_channels)) __perf_ret =
      ...
      // Write data
      fprintf(f_csv, "size_from_channelarray,%lu,%lu,%lu,%lu newline",
