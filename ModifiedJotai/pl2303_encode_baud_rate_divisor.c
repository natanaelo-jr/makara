...
int main(int argc, char *argv[]) {
    ...
    if (fd_cache_miss == -1) { perror("perf_event_open (cache misses)"); exit(1); }
          ...
          __typeof__(pl2303_encode_baud_rate_divisor(buf,baud)) __perf_ret = pl2303_encode_baud_rate_divisor(buf,baud);
    ...
    // Write data
    fprintf(f_csv, "pl2303_encode_baud_rate_divisor,%lu,%lu,%lu,%lu newline",
            count_cycles,
            count_instructions,
            count_cache_miss,
            count_cache_ref);
