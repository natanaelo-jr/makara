...
struct TYPE_2__ {
  ...
  int /*<<< orphan*/ interleaving_style;
  int /*<<< orphan*/ ch_map;
  int /*<<< orphan*/ ch_cfg;
  int /*<<< orphan*/ valid_bit_depth;
...
skl_tplg_fill_fmt_v4(struct skl_module_pin_fmt *dst_fmt,
                     struct skl_dfw_v4_module_fmt *src_fmt, int pins) {
  int i;
  ...
  for (i = 0; i < pins; i++) {
    ...
    dst_fmt[i].fmt.interleaving_style = src_fmt[i].interleaving_style;
...
int main(int argc, char *argv[]) {
  ...
  case 0: {
    ...
    struct skl_module_pin_fmt *dst_fmt = (struct skl_module_pin_fmt *)malloc(
        _len_dst_fmt0 * sizeof(struct skl_module_pin_fmt));
    for (int _i0 = 0; _i0 < _len_dst_fmt0; _i0++) {
      dst_fmt[_i0].fmt.sample_type = ((-2 * (next_i() % 2)) + 1) * next_i();
      dst_fmt[_i0].fmt.interleaving_style =
          ((-2 * (next_i() % 2)) + 1) * next_i();
    ...
    struct skl_dfw_v4_module_fmt *src_fmt =
        (struct skl_dfw_v4_module_fmt *)malloc(
            _len_src_fmt0 * sizeof(struct skl_dfw_v4_module_fmt));
...

    {
      ...
      // Write data
      fprintf(f_csv, "skl_tplg_fill_fmt_v4,%lu,%lu,%lu,%lu newline",
