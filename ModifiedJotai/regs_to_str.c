...
__attribute__((used)) static void regs_to_str(unsigned int *regs,
                                              unsigned int len, WCHAR *buffer) {
  unsigned int i;
...
int main(int argc, char *argv[]) {
  ...
  case 0: {
    ...
    unsigned int *regs =
        (unsigned int *)malloc(_len_regs0 * sizeof(unsigned int));
...

    {
      ...
      // Write data
      fprintf(f_csv, "regs_to_str,%lu,%lu,%lu,%lu newline", count_cycles,
