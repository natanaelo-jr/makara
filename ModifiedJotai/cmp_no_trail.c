...
__attribute__((used)) static int cmp_no_trail(u8 *a, u8 *b,
                                              unsigned int count) {
  while (--count) {
    if (*a++ != *b++)
      return 1;
...
int main(int argc, char *argv[]) {
  ...
  case 0: {
    ...
    int benchRet = ({
      ...
      __typeof__(cmp_no_trail(a, b, count)) __perf_ret =
          cmp_no_trail(a, b, count);
      ...
      // Write data
      fprintf(f_csv, "cmp_no_trail,%lu,%lu,%lu,%lu newline", count_cycles,
