...
__attribute__((used)) static u8 dccp_feat_prefer(u8 preferred_value, u8 *array,
                                                 u8 array_len) {
  u8 i, does_occur = 0;
...
int main(int argc, char *argv[]) {
  ...
  case 0: {
    ...
    int _len_array0 = 100;
    unsigned long *array =
        (unsigned long *)malloc(_len_array0 * sizeof(unsigned long));
    ...
    unsigned long benchRet = ({
      ...
      __typeof__(dccp_feat_prefer(preferred_value, array,
                                  array_len)) __perf_ret =
          dccp_feat_prefer(preferred_value, array, array_len);
      ...
      // Write data
      fprintf(f_csv, "dccp_feat_prefer,%lu,%lu,%lu,%lu newline", count_cycles,
