...
__attribute__((used)) static int repair_bit(unsigned char *input, int l,
                                            int k) {
  if (k < 0) {
    return -1;
  }
...
int main(int argc, char *argv[]) {
  ...
  case 0: {
    ...
    int _len_input0 = 100;
    unsigned char *input =
        (unsigned char *)malloc(_len_input0 * sizeof(unsigned char));
    ...
    int benchRet = ({
      ...
      __typeof__(repair_bit(input, l, k)) __perf_ret = repair_bit(input, l, k);
      ...
      // Write data
      fprintf(f_csv, "repair_bit,%lu,%lu,%lu,%lu newline", count_cycles,
