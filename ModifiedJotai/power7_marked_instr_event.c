...
__attribute__((used)) static int power7_marked_instr_event(u64 mmcr1) {
   ...
   */
  for (pmc = 0; pmc < 4; pmc++) {
    psel = mmcr1 & (OPROFILE_PM_PMCSEL_MSK << (OPROFILE_MAX_PMC_NUM - pmc) *
                                                  OPROFILE_PMSEL_FIELD_WIDTH);
    psel =
        (psel >> ((OPROFILE_MAX_PMC_NUM - pmc) * OPROFILE_PMSEL_FIELD_WIDTH)) &
        ~1ULL;
    unit = mmcr1 &
           (OPROFILE_PM_UNIT_MSK
            << (OPROFILE_PM_UNIT_SHIFT - (pmc * OPROFILE_PMSEL_FIELD_WIDTH)));
    unit =
        unit >> (OPROFILE_PM_UNIT_SHIFT - (pmc * OPROFILE_PMSEL_FIELD_WIDTH));
...
int main(int argc, char *argv[]) {
  ...
  case 0: {
    ...
    int benchRet = ({
      ...
      __typeof__(power7_marked_instr_event(mmcr1)) __perf_ret =
          power7_marked_instr_event(mmcr1);
      ...
      // Write data
      fprintf(f_csv, "power7_marked_instr_event,%lu,%lu,%lu,%lu newline",
