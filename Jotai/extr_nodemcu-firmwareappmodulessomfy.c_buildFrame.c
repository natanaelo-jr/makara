...
void buildFrame(uint8_t *frame, uint64_t remote, uint8_t button,
                uint16_t code) {
  // NODE_DBG("remote: %x\n", remote);
  // NODE_DBG("button: %x\n", button);
  ...
  // frame[1], frame[2], frame[3], frame[4], frame[5], frame[6]); Checksum
  // calculation: a XOR of all the nibbles
  uint8_t checksum = 0;
