...
int main(int argc, char *argv[]) {
    ...
    if (fd_cache_miss == -1) { perror("perf_event_open (cache misses)"); exit(1); }
          ...
          __typeof__(has_channel(channel_set,chanset_size,chan)) __perf_ret = has_channel(channel_set,chanset_size,chan);
    ...
    // Write data
    fprintf(f_csv, "has_channel,%lu,%lu,%lu,%lu newline",
            count_cycles,
            count_instructions,
            count_cache_miss,
            count_cache_ref);
