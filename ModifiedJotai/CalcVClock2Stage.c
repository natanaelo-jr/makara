...
__attribute__((used)) static void CalcVClock2Stage(int clockIn, int *clockOut,
                                                   u32 *pllOut, u32 *pllBOut,
  ...
  *pllBOut = 0x80000401; /* fixed at x4 for now */
  ...
  for (P = 0; P <= 6; P++) {
    ...
    if ((Freq >= 400000) && (Freq <= 1000000)) {
      for (M = 1; M <= 13; M++) {
        N = ((VClk << P) * M) / (par->CrystalFreqKHz << 2);
        if ((N >= 5) && (N <= 255)) {
          Freq = (((par->CrystalFreqKHz << 2) * N) / M) >> P;
          if (Freq > VClk)
            DeltaNew = Freq - VClk;
          else
            DeltaNew = VClk - Freq;
...
int main(int argc, char *argv[]) {
  ...
  case 0: {
    ...
    int _len_par0 = 1;
    struct nvidia_par *par =
        (struct nvidia_par *)malloc(_len_par0 * sizeof(struct nvidia_par));
    ...
    }
    {
      ...
      // Write data
      fprintf(f_csv, "CalcVClock2Stage,%lu,%lu,%lu,%lu newline", count_cycles,
