...
int main(int argc, char *argv[]) {
    ...
    if (fd_cache_miss == -1) { perror("perf_event_open (cache misses)"); exit(1); }
          ...
          __typeof__(__perf_evsel__sample_size(sample_type)) __perf_ret = __perf_evsel__sample_size(sample_type);
    ...
    // Write data
    fprintf(f_csv, "__perf_evsel__sample_size,%lu,%lu,%lu,%lu newline",
            count_cycles,
            count_instructions,
            count_cache_miss,
            count_cache_ref);
