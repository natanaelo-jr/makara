...
pm3fb_calculate_clock(unsigned long reqclock, unsigned char *prescale,
                      unsigned char *feedback, unsigned char *postscale) {
  int f, pre, post;
  unsigned long freq;
  long freqerr = 1000;
  long currerr;
  ...
  for (f = 1; f < 256; f++) {
    for (pre = 1; pre < 256; pre++) {
      for (post = 0; post < 5; post++) {
        ...
        currerr = (reqclock > freq) ? reqclock - freq : freq - reqclock;
        if (currerr < freqerr) {
          freqerr = currerr;
          *feedback = f;
          *prescale = pre;
...
int main(int argc, char *argv[]) {
  ...
  case 0: {
    ...
    unsigned char *prescale =
        (unsigned char *)malloc(_len_prescale0 * sizeof(unsigned char));
    ...
    unsigned char *feedback =
        (unsigned char *)malloc(_len_feedback0 * sizeof(unsigned char));
    ...
    unsigned char *postscale =
        (unsigned char *)malloc(_len_postscale0 * sizeof(unsigned char));
...

    {
      ...
      // Write data
      fprintf(f_csv, "pm3fb_calculate_clock,%lu,%lu,%lu,%lu newline",
