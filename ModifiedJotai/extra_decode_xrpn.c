...
__attribute__((used)) static int extra_decode_xrpn(struct snd_midi_event *dev,
                                                   unsigned char *buf,
                                                   int count,
                                                   struct snd_seq_event *ev) {
  unsigned char cmd;
  char *cbytes;
  static char cbytes_nrpn[4] = {
      MIDI_CTL_NONREG_PARM_NUM_MSB, MIDI_CTL_NONREG_PARM_NUM_LSB,
      MIDI_CTL_MSB_DATA_ENTRY, MIDI_CTL_LSB_DATA_ENTRY};
  static char cbytes_rpn[4] = {
      MIDI_CTL_REGIST_PARM_NUM_MSB, MIDI_CTL_REGIST_PARM_NUM_LSB,
      MIDI_CTL_MSB_DATA_ENTRY, MIDI_CTL_LSB_DATA_ENTRY};
...
int main(int argc, char *argv[]) {
  ...
  case 0: {
    ...
    int _len_dev0 = 65025;
    struct snd_midi_event *dev = (struct snd_midi_event *)malloc(
        _len_dev0 * sizeof(struct snd_midi_event));
    ...
    int _len_ev0 = 65025;
    struct snd_seq_event *ev =
        (struct snd_seq_event *)malloc(_len_ev0 * sizeof(struct snd_seq_event));
    ...
    int benchRet = ({
      ...
      __typeof__(extra_decode_xrpn(dev, buf, count, ev)) __perf_ret =
          extra_decode_xrpn(dev, buf, count, ev);
      ...
      // Write data
      fprintf(f_csv, "extra_decode_xrpn,%lu,%lu,%lu,%lu newline", count_cycles,
