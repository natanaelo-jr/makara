...
u8 cx231xx_find_next_SAV_EAV(u8 *p_buffer, u32 buffer_size, u32 *p_bytes_used) {
  ...
  for (i = 0; i < (buffer_size - 3); i++) {
    ...
    if ((p_buffer[i] == 0xFF) && (p_buffer[i + 1] == 0x00) &&
        (p_buffer[i + 2] == 0x00)) {
...
int main(int argc, char *argv[]) {
  ...
  case 0: {
    ...
    int benchRet = ({
      ...
      __typeof__(cx231xx_find_next_SAV_EAV(p_buffer, buffer_size,
                                           p_bytes_used)) __perf_ret =
          cx231xx_find_next_SAV_EAV(p_buffer, buffer_size, p_bytes_used);
      ...
      // Write data
      fprintf(f_csv, "cx231xx_find_next_SAV_EAV,%lu,%lu,%lu,%lu newline",
