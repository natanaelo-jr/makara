...
int main(int argc, char *argv[]) {
    ...
    if (fd_cache_miss == -1) { perror("perf_event_open (cache misses)"); exit(1); }
          ...
          __typeof__(pcf_has_table_type(tables,ntables,type)) __perf_ret = pcf_has_table_type(tables,ntables,type);
    ...
    // Write data
    fprintf(f_csv, "pcf_has_table_type,%lu,%lu,%lu,%lu newline",
            count_cycles,
            count_instructions,
            count_cache_miss,
            count_cache_ref);
