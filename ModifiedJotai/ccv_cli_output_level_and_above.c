...
int main(int argc, char *argv[]) {
    ...
    if (fd_cache_miss == -1) { perror("perf_event_open (cache misses)"); exit(1); }
          ...
          __typeof__(ccv_cli_output_level_and_above(level)) __perf_ret = ccv_cli_output_level_and_above(level);
    ...
    // Write data
    fprintf(f_csv, "ccv_cli_output_level_and_above,%lu,%lu,%lu,%lu newline",
            count_cycles,
            count_instructions,
            count_cache_miss,
            count_cache_ref);
