...
PKULL_M_PATCH_GENERIC
kull_m_patch_getGenericFromBuild(PKULL_M_PATCH_GENERIC generics,
                                 SIZE_T cbGenerics, DWORD BuildNumber) {
  SIZE_T i;
...
int main(int argc, char *argv[]) {
  ...
  case 0: {
    ...
    struct TYPE_4__ *generics =
        (struct TYPE_4__ *)malloc(_len_generics0 * sizeof(struct TYPE_4__));
    for (int _i0 = 0; _i0 < _len_generics0; _i0++) {
      generics[_i0].MinBuildNumber = ((-2 * (next_i() % 2)) + 1) * next_i();
    }
    struct TYPE_4__ *benchRet = ({
      ...
      __typeof__(kull_m_patch_getGenericFromBuild(generics, cbGenerics,
                                                  BuildNumber)) __perf_ret =
          kull_m_patch_getGenericFromBuild(generics, cbGenerics, BuildNumber);
      ...
      // Write data
      fprintf(f_csv, "kull_m_patch_getGenericFromBuild,%lu,%lu,%lu,%lu newline",
