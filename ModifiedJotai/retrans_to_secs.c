...
int main(int argc, char *argv[]) {
    ...
    if (fd_cache_miss == -1) { perror("perf_event_open (cache misses)"); exit(1); }
          ...
          __typeof__(retrans_to_secs(retrans,timeout,rto_max)) __perf_ret = retrans_to_secs(retrans,timeout,rto_max);
    ...
    // Write data
    fprintf(f_csv, "retrans_to_secs,%lu,%lu,%lu,%lu newline",
            count_cycles,
            count_instructions,
            count_cache_miss,
            count_cache_ref);
