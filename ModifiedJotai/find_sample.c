...
__attribute__((used)) static int find_sample(struct sample *samples,
                                             size_t nr_samples,
                                             struct thread *t, struct map *m,
                                             struct symbol *s) {
  while (nr_samples--) {
    if (samples->thread == t && samples->map == m && samples->sym == s)
      return 1;
    samples++;
  }
  return 0;
...
int main(int argc, char *argv[]) {
  ...
  case 0: {
    ...
    struct sample *samples =
        (struct sample *)malloc(_len_samples0 * sizeof(struct sample));
    for (int _i0 = 0; _i0 < _len_samples0; _i0++) {
      int _len_samples__i0__sym0 = 1;
      samples[_i0].sym = (struct symbol *)malloc(_len_samples__i0__sym0 *
      ...
      samples[_i0].map =
          (struct map *)malloc(_len_samples__i0__map0 * sizeof(struct map));
      for (int _j0 = 0; _j0 < _len_samples__i0__map0; _j0++) {
        samples[_i0].map->dummy = ((-2 * (next_i() % 2)) + 1) * next_i();
      }
      int _len_samples__i0__thread0 = 1;
      samples[_i0].thread = (struct thread *)malloc(_len_samples__i0__thread0 *
    ...
    int benchRet = ({
      ...
      __typeof__(find_sample(samples, nr_samples, t, m, s)) __perf_ret =
          find_sample(samples, nr_samples, t, m, s);
      ...
      // Write data
      fprintf(f_csv, "find_sample,%lu,%lu,%lu,%lu newline", count_cycles,
