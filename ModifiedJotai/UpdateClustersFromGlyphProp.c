...
UpdateClustersFromGlyphProp(const int cGlyphs, const int cChars,
                            WORD *pwLogClust, SCRIPT_GLYPHPROP *pGlyphProp) {
  int i;
    ...
    if (!pGlyphProp[i].sva.fClusterStart) {
      ...
      for (j = 0; j < cChars; j++) {
        if (pwLogClust[j] == i) {
          int k = j;
          while (k >= 0 && k < cChars &&
                 !pGlyphProp[pwLogClust[k]].sva.fClusterStart)
            k -= 1;
          if (k >= 0 && k < cChars &&
              pGlyphProp[pwLogClust[k]].sva.fClusterStart)
...
int main(int argc, char *argv[]) {
  ...
  case 0: {
    ...
    struct TYPE_5__ *pGlyphProp =
        (struct TYPE_5__ *)malloc(_len_pGlyphProp0 * sizeof(struct TYPE_5__));
    for (int _i0 = 0; _i0 < _len_pGlyphProp0; _i0++) {
      pGlyphProp[_i0].sva.fClusterStart =
          ((-2 * (next_i() % 2)) + 1) * next_i();
    }

    {
      ...
      // Write data
      fprintf(f_csv, "UpdateClustersFromGlyphProp,%lu,%lu,%lu,%lu newline",
