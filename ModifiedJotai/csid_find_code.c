...
int main(int argc, char *argv[]) {
    ...
    if (fd_cache_miss == -1) { perror("perf_event_open (cache misses)"); exit(1); }
          ...
          __typeof__(csid_find_code(code,n_code,index,req_code)) __perf_ret = csid_find_code(code,n_code,index,req_code);
    ...
    // Write data
    fprintf(f_csv, "csid_find_code,%lu,%lu,%lu,%lu newline",
            count_cycles,
            count_instructions,
            count_cache_miss,
            count_cache_ref);
