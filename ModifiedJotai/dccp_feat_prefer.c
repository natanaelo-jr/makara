...
int main(int argc, char *argv[]) {
    ...
    if (fd_cache_miss == -1) { perror("perf_event_open (cache misses)"); exit(1); }
          ...
          __typeof__(dccp_feat_prefer(preferred_value,array,array_len)) __perf_ret = dccp_feat_prefer(preferred_value,array,array_len);
    ...
    // Write data
    fprintf(f_csv, "dccp_feat_prefer,%lu,%lu,%lu,%lu newline",
            count_cycles,
            count_instructions,
            count_cache_miss,
            count_cache_ref);
