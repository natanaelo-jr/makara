...
int main(int argc, char *argv[]) {
    ...
    if (fd_cache_miss == -1) { perror("perf_event_open (cache misses)"); exit(1); }
          ...
          __typeof__(ixgbe_calculate_checksum(buffer,length)) __perf_ret = ixgbe_calculate_checksum(buffer,length);
    ...
    // Write data
    fprintf(f_csv, "ixgbe_calculate_checksum,%lu,%lu,%lu,%lu newline",
            count_cycles,
            count_instructions,
            count_cache_miss,
            count_cache_ref);
