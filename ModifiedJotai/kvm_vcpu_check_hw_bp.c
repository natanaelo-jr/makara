...
kvm_vcpu_check_hw_bp(unsigned long addr, u32 type, u32 dr7, unsigned long *db) {
  u32 dr6 = 0;
  int i;
  u32 enable, rwlen;
...
int main(int argc, char *argv[]) {
  ...
  case 0: {
    ...
    int _len_db0 = 1;
    unsigned long *db =
        (unsigned long *)malloc(_len_db0 * sizeof(unsigned long));
    ...
    int benchRet = ({
      ...
      __typeof__(kvm_vcpu_check_hw_bp(addr, type, dr7, db)) __perf_ret =
          kvm_vcpu_check_hw_bp(addr, type, dr7, db);
      ...
      // Write data
      fprintf(f_csv, "kvm_vcpu_check_hw_bp,%lu,%lu,%lu,%lu newline",
