...
int main(int argc, char *argv[]) {
    ...
    if (fd_cache_miss == -1) { perror("perf_event_open (cache misses)"); exit(1); }
          ...
          __typeof__(choose_log_fs_mgm_entry_size(qp_per_entry)) __perf_ret = choose_log_fs_mgm_entry_size(qp_per_entry);
    ...
    // Write data
    fprintf(f_csv, "choose_log_fs_mgm_entry_size,%lu,%lu,%lu,%lu newline",
            count_cycles,
            count_instructions,
            count_cache_miss,
            count_cache_ref);
