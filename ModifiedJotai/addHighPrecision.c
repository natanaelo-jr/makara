...
int main(int argc, char *argv[]) {
    ...
    if (fd_cache_miss == -1) { perror("perf_event_open (cache misses)"); exit(1); }
          ...
          __typeof__(addHighPrecision(arg1,length1,arg2,length2)) __perf_ret = addHighPrecision(arg1,length1,arg2,length2);
    ...
    // Write data
    fprintf(f_csv, "addHighPrecision,%lu,%lu,%lu,%lu newline",
            count_cycles,
            count_instructions,
            count_cache_miss,
            count_cache_ref);
