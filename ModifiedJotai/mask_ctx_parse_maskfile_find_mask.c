...
int main(int argc, char *argv[]) {
    ...
    if (fd_cache_miss == -1) { perror("perf_event_open (cache misses)"); exit(1); }
          ...
          __typeof__(mask_ctx_parse_maskfile_find_mask(line_buf,line_len)) __perf_ret = mask_ctx_parse_maskfile_find_mask(line_buf,line_len);
    ...
    // Write data
    fprintf(f_csv, "mask_ctx_parse_maskfile_find_mask,%lu,%lu,%lu,%lu newline",
            count_cycles,
            count_instructions,
            count_cache_miss,
            count_cache_ref);
