...
munge_int32(unsigned int src, unsigned char *dst, unsigned int dst_size) {
  unsigned int i;
...
int main(int argc, char *argv[]) {
  ...
  case 0: {
    ...
    int _len_dst0 = 100;
    unsigned char *dst =
        (unsigned char *)malloc(_len_dst0 * sizeof(unsigned char));
    ...
    unsigned char *benchRet = ({
      ...
      __typeof__(munge_int32(src, dst, dst_size)) __perf_ret =
          munge_int32(src, dst, dst_size);
      ...
      // Write data
      fprintf(f_csv, "munge_int32,%lu,%lu,%lu,%lu newline", count_cycles,
