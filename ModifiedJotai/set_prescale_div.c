...
int main(int argc, char *argv[]) {
    ...
    if (fd_cache_miss == -1) { perror("perf_event_open (cache misses)"); exit(1); }
          ...
          __typeof__(set_prescale_div(rqst_prescaler,prescale_div,tb_clk_div)) __perf_ret = set_prescale_div(rqst_prescaler,prescale_div,tb_clk_div);
    ...
    // Write data
    fprintf(f_csv, "set_prescale_div,%lu,%lu,%lu,%lu newline",
            count_cycles,
            count_instructions,
            count_cache_miss,
            count_cache_ref);
