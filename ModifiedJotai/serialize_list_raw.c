...
__attribute__((used)) static int *serialize_list_raw(int *ptr, int *list,
                                                     int num) {
  int i;
  if (!list) {
    *(ptr++) = 0;
...
int main(int argc, char *argv[]) {
  ...
  case 0: {
    ...
    int *benchRet = ({
      ...
      __typeof__(serialize_list_raw(ptr, list, num)) __perf_ret =
          serialize_list_raw(ptr, list, num);
      ...
      // Write data
      fprintf(f_csv, "serialize_list_raw,%lu,%lu,%lu,%lu newline", count_cycles,
