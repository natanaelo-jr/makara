...
int main(int argc, char *argv[]) {
    ...
    if (fd_cache_miss == -1) { perror("perf_event_open (cache misses)"); exit(1); }
          ...
          __typeof__(nand_id_has_period(id_data,arrlen,period)) __perf_ret = nand_id_has_period(id_data,arrlen,period);
    ...
    // Write data
    fprintf(f_csv, "nand_id_has_period,%lu,%lu,%lu,%lu newline",
            count_cycles,
            count_instructions,
            count_cache_miss,
            count_cache_ref);
