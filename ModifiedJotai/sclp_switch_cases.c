...
__attribute__((used)) static int sclp_switch_cases(unsigned char *buf,
                                                   int count) {
  unsigned char *ip, *op;
  int toggle;
...
int main(int argc, char *argv[]) {
  ...
  case 0: {
    ...
    int benchRet = ({
      ...
      __typeof__(sclp_switch_cases(buf, count)) __perf_ret =
          sclp_switch_cases(buf, count);
      ...
      // Write data
      fprintf(f_csv, "sclp_switch_cases,%lu,%lu,%lu,%lu newline", count_cycles,
