...
int main(int argc, char *argv[]) {
    ...
    if (fd_cache_miss == -1) { perror("perf_event_open (cache misses)"); exit(1); }
          ...
          __typeof__(find_next(sline,mark,i,cnt,look_for_uninteresting)) __perf_ret = find_next(sline,mark,i,cnt,look_for_uninteresting);
    ...
    // Write data
    fprintf(f_csv, "find_next,%lu,%lu,%lu,%lu newline",
            count_cycles,
            count_instructions,
            count_cache_miss,
            count_cache_ref);
