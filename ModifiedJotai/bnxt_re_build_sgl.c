...
int main(int argc, char *argv[]) {
    ...
    if (fd_cache_miss == -1) { perror("perf_event_open (cache misses)"); exit(1); }
          ...
          __typeof__(bnxt_re_build_sgl(ib_sg_list,sg_list,num)) __perf_ret = bnxt_re_build_sgl(ib_sg_list,sg_list,num);
    ...
    // Write data
    fprintf(f_csv, "bnxt_re_build_sgl,%lu,%lu,%lu,%lu newline",
            count_cycles,
            count_instructions,
            count_cache_miss,
            count_cache_ref);
