...
int main(int argc, char *argv[]) {
    ...
    if (fd_cache_miss == -1) { perror("perf_event_open (cache misses)"); exit(1); }
          ...
          __typeof__(FindActionIndex(lpAction,dwActionSize,poAction)) __perf_ret = FindActionIndex(lpAction,dwActionSize,poAction);
    ...
    // Write data
    fprintf(f_csv, "FindActionIndex,%lu,%lu,%lu,%lu newline",
            count_cycles,
            count_instructions,
            count_cache_miss,
            count_cache_ref);
