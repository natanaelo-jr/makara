...
int main(int argc, char *argv[]) {
    ...
    if (fd_cache_miss == -1) { perror("perf_event_open (cache misses)"); exit(1); }
          ...
          __typeof__(sfnt_find_encoding(platform_id,encoding_id)) __perf_ret = sfnt_find_encoding(platform_id,encoding_id);
    ...
    // Write data
    fprintf(f_csv, "sfnt_find_encoding,%lu,%lu,%lu,%lu newline",
            count_cycles,
            count_instructions,
            count_cache_miss,
            count_cache_ref);
