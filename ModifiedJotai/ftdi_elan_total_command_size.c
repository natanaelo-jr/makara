...
ftdi_elan_total_command_size(struct usb_ftdi *ftdi, int command_size) {
  int total_size = 0;
  int I = command_size;
  int i = ftdi->command_head;
  while (I-- > 0) {
    struct u132_command *command = &ftdi->command[COMMAND_MASK & i++];
...
int main(int argc, char *argv[]) {
  ...
  case 0: {
    ...
    struct usb_ftdi *ftdi =
        (struct usb_ftdi *)malloc(_len_ftdi0 * sizeof(struct usb_ftdi));
    for (int _i0 = 0; _i0 < _len_ftdi0; _i0++) {
      ftdi[_i0].command_head = ((-2 * (next_i() % 2)) + 1) * next_i();
      int _len_ftdi__i0__command0 = 1;
      ftdi[_i0].command = (struct u132_command *)malloc(
          _len_ftdi__i0__command0 * sizeof(struct u132_command));
    ...
    int benchRet = ({
      ...
      __typeof__(ftdi_elan_total_command_size(ftdi, command_size)) __perf_ret =
          ftdi_elan_total_command_size(ftdi, command_size);
      ...
      // Write data
      fprintf(f_csv, "ftdi_elan_total_command_size,%lu,%lu,%lu,%lu newline",
