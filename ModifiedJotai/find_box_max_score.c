...
__attribute__((used)) static struct box *find_box_max_score(struct box *b,
                                                            int count) {
  struct box *best = NULL;
  for (; count--; b++)
    if (b->score && (!best || b->score > best->score))
      best = b;
  return best;
}
...
int main(int argc, char *argv[]) {
  ...
  case 0: {
    ...
    struct box *benchRet = ({
      ...
      __typeof__(find_box_max_score(b, count)) __perf_ret =
          find_box_max_score(b, count);
      ...
      // Write data
      fprintf(f_csv, "find_box_max_score,%lu,%lu,%lu,%lu newline", count_cycles,
