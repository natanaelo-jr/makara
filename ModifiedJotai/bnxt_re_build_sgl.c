...
bnxt_re_build_sgl(struct ib_sge *ib_sg_list, struct bnxt_qplib_sge *sg_list,
                  int num) {
  int i, total = 0;
...
int main(int argc, char *argv[]) {
  ...
  case 0: {
    ...
    struct ib_sge *ib_sg_list =
        (struct ib_sge *)malloc(_len_ib_sg_list0 * sizeof(struct ib_sge));
    ...
    struct bnxt_qplib_sge *sg_list = (struct bnxt_qplib_sge *)malloc(
        _len_sg_list0 * sizeof(struct bnxt_qplib_sge));
    ...
    int benchRet = ({
      ...
      __typeof__(bnxt_re_build_sgl(ib_sg_list, sg_list, num)) __perf_ret =
          bnxt_re_build_sgl(ib_sg_list, sg_list, num);
      ...
      // Write data
      fprintf(f_csv, "bnxt_re_build_sgl,%lu,%lu,%lu,%lu newline", count_cycles,
