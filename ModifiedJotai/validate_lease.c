...
int main(int argc, char *argv[]) {
    ...
    if (fd_cache_miss == -1) { perror("perf_event_open (cache misses)"); exit(1); }
          ...
          __typeof__(validate_lease(dev,lessor_priv,object_count,objects)) __perf_ret = validate_lease(dev,lessor_priv,object_count,objects);
    ...
    // Write data
    fprintf(f_csv, "validate_lease,%lu,%lu,%lu,%lu newline",
            count_cycles,
            count_instructions,
            count_cache_miss,
            count_cache_ref);
