...
int main(int argc, char *argv[]) {
    ...
    if (fd_cache_miss == -1) { perror("perf_event_open (cache misses)"); exit(1); }
          ...
          __typeof__(row_shift_from_symbol_count(symbol_count)) __perf_ret = row_shift_from_symbol_count(symbol_count);
    ...
    // Write data
    fprintf(f_csv, "row_shift_from_symbol_count,%lu,%lu,%lu,%lu newline",
            count_cycles,
            count_instructions,
            count_cache_miss,
            count_cache_ref);
