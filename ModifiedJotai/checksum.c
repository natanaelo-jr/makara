...
__attribute__((used)) static inline unsigned long checksum(unsigned long *addr,
                                                           int len) {
...
int main(int argc, char *argv[]) {
  ...
  case 0: {
    ...
    int _len_addr0 = 100;
    unsigned long *addr =
        (unsigned long *)malloc(_len_addr0 * sizeof(unsigned long));
    ...
    unsigned long benchRet = ({
      ...
      __typeof__(checksum(addr, len)) __perf_ret = checksum(addr, len);
      ...
      // Write data
      fprintf(f_csv, "checksum,%lu,%lu,%lu,%lu newline", count_cycles,
