...
__attribute__((used)) static inline int dle_count(unsigned char *buf, int len) {
  int count = 0;
...
int main(int argc, char *argv[]) {
  ...
  case 0: {
    ...
    int benchRet = ({
      ...
      __typeof__(dle_count(buf, len)) __perf_ret = dle_count(buf, len);
      ...
      // Write data
      fprintf(f_csv, "dle_count,%lu,%lu,%lu,%lu newline", count_cycles,
