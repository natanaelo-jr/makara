...
int main(int argc, char *argv[]) {
    ...
    if (fd_cache_miss == -1) { perror("perf_event_open (cache misses)"); exit(1); }
          ...
          __typeof__(mpihelp_sub_n(res_ptr,s1_ptr,s2_ptr,size)) __perf_ret = mpihelp_sub_n(res_ptr,s1_ptr,s2_ptr,size);
    ...
    // Write data
    fprintf(f_csv, "mpihelp_sub_n,%lu,%lu,%lu,%lu newline",
            count_cycles,
            count_instructions,
            count_cache_miss,
            count_cache_ref);
