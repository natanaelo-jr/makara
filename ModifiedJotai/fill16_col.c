...
__attribute__((used)) static inline u32 *fill16_col(u32 *dst, int rows,
                                                    u32 m[]) {
  while (rows) {
    *dst++ = m[0];
#if BPL > 2
...
int main(int argc, char *argv[]) {
  ...
  case 0: {
    ...
    int *benchRet = ({
      ...
      __typeof__(fill16_col(dst, rows, m)) __perf_ret =
          fill16_col(dst, rows, m);
      ...
      // Write data
      fprintf(f_csv, "fill16_col,%lu,%lu,%lu,%lu newline", count_cycles,
