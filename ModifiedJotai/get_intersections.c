...
int get_intersections(double *lines, double *intersections, long long n) {
  ...
  for (int i = 0; i < n; i++) {
    for (int j = 0; j < n; j++) {
      ...
      // only intersect lines from different quadrants and only left-right
      // crossing
...
int main(int argc, char *argv[]) {
  ...
  case 0: {
    ...
    double *intersections =
        (double *)malloc(_len_intersections0 * sizeof(double));
    ...
    int benchRet = ({
      ...
      __typeof__(get_intersections(lines, intersections, n)) __perf_ret =
          get_intersections(lines, intersections, n);
      ...
      // Write data
      fprintf(f_csv, "get_intersections,%lu,%lu,%lu,%lu newline", count_cycles,
