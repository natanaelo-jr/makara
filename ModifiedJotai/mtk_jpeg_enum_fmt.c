...
int main(int argc, char *argv[]) {
    ...
    if (fd_cache_miss == -1) { perror("perf_event_open (cache misses)"); exit(1); }
          ...
          __typeof__(mtk_jpeg_enum_fmt(mtk_jpeg_formats,n,f,type)) __perf_ret = mtk_jpeg_enum_fmt(mtk_jpeg_formats,n,f,type);
    ...
    // Write data
    fprintf(f_csv, "mtk_jpeg_enum_fmt,%lu,%lu,%lu,%lu newline",
            count_cycles,
            count_instructions,
            count_cache_miss,
            count_cache_ref);
