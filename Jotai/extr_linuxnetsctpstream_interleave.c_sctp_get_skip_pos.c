...
sctp_get_skip_pos(struct sctp_ifwdtsn_skip *skiplist, int nskips, __be16 stream,
                  __u8 flags) {
  int i;

  for (i = 0; i < nskips; i++)
    if (skiplist[i].stream == stream && skiplist[i].flags == flags)
      return i;
...
int main(int argc, char *argv[]) {
  ...
  case 0: {
    ...
    struct sctp_ifwdtsn_skip *skiplist = (struct sctp_ifwdtsn_skip *)malloc(
        _len_skiplist0 * sizeof(struct sctp_ifwdtsn_skip));
