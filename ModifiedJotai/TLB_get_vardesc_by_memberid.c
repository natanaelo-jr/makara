...
int main(int argc, char *argv[]) {
    ...
    if (fd_cache_miss == -1) { perror("perf_event_open (cache misses)"); exit(1); }
          ...
          __typeof__(TLB_get_vardesc_by_memberid(vardescs,n,memid)) __perf_ret = TLB_get_vardesc_by_memberid(vardescs,n,memid);
    ...
    // Write data
    fprintf(f_csv, "TLB_get_vardesc_by_memberid,%lu,%lu,%lu,%lu newline",
            count_cycles,
            count_instructions,
            count_cache_miss,
            count_cache_ref);
