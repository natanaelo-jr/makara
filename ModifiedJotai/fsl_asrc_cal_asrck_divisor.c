...
fsl_asrc_cal_asrck_divisor(struct fsl_asrc_pair *pair, u32 div) {
  u32 ps;
...
int main(int argc, char *argv[]) {
  ...
  case 0: {
    int div = 100;
    int _len_pair0 = 1;
    struct fsl_asrc_pair *pair = (struct fsl_asrc_pair *)malloc(
        _len_pair0 * sizeof(struct fsl_asrc_pair));
    ...
    int benchRet = ({
      ...
      __typeof__(fsl_asrc_cal_asrck_divisor(pair, div)) __perf_ret =
          fsl_asrc_cal_asrck_divisor(pair, div);
      ...
      // Write data
      fprintf(f_csv, "fsl_asrc_cal_asrck_divisor,%lu,%lu,%lu,%lu newline",
