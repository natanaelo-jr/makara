...
int main(int argc, char *argv[]) {
    ...
    if (fd_cache_miss == -1) { perror("perf_event_open (cache misses)"); exit(1); }
          ...
          __typeof__(fixpt_div32(dividend,divisor,quotient,fraction)) __perf_ret = fixpt_div32(dividend,divisor,quotient,fraction);
    ...
    // Write data
    fprintf(f_csv, "fixpt_div32,%lu,%lu,%lu,%lu newline",
            count_cycles,
            count_instructions,
            count_cache_miss,
            count_cache_ref);
