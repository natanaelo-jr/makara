...
freq_table_redup(struct cpufreq_frequency_table *freq_table, int count) {
  int i, j;

  for (i = 1; i < count; i++) {
    for (j = 0; j < i; j++) {
      if (freq_table[j].frequency == CPUFREQ_ENTRY_INVALID ||
          freq_table[j].frequency != freq_table[i].frequency)
        continue;
...
int main(int argc, char *argv[]) {
  ...
  case 0: {
    ...
    struct cpufreq_frequency_table *freq_table =
        (struct cpufreq_frequency_table *)malloc(
            _len_freq_table0 * sizeof(struct cpufreq_frequency_table));
    ...
    }
    {
      ...
      // Write data
      fprintf(f_csv, "freq_table_redup,%lu,%lu,%lu,%lu newline", count_cycles,
