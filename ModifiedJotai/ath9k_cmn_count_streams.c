...
int main(int argc, char *argv[]) {
    ...
    if (fd_cache_miss == -1) { perror("perf_event_open (cache misses)"); exit(1); }
          ...
          __typeof__(ath9k_cmn_count_streams(chainmask,max)) __perf_ret = ath9k_cmn_count_streams(chainmask,max);
    ...
    // Write data
    fprintf(f_csv, "ath9k_cmn_count_streams,%lu,%lu,%lu,%lu newline",
            count_cycles,
            count_instructions,
            count_cache_miss,
            count_cache_ref);
