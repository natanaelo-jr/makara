...
get_vlarb_preempt(struct hfi1_devdata *dd, u32 nelems,
...
int main(int argc, char *argv[]) {
  ...
  case 0: {
    ...
    struct hfi1_devdata *dd =
        (struct hfi1_devdata *)malloc(_len_dd0 * sizeof(struct hfi1_devdata));
    for (int _i0 = 0; _i0 < _len_dd0; _i0++) {
      dd[_i0].dummy = ((-2 * (next_i() % 2)) + 1) * next_i();
    }

    int _len_vl0 = 65025;
    struct ib_vl_weight_elem *vl = (struct ib_vl_weight_elem *)malloc(
        _len_vl0 * sizeof(struct ib_vl_weight_elem));
...

    {
      ...
      // Write data
      fprintf(f_csv, "get_vlarb_preempt,%lu,%lu,%lu,%lu newline", count_cycles,
