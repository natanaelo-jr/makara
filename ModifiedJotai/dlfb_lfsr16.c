...
__attribute__((used)) static u16 dlfb_lfsr16(u16 actual_count) {
  ...
  while (actual_count--) {
    lv = ((lv << 1) | (((lv >> 15) ^ (lv >> 4) ^ (lv >> 2) ^ (lv >> 1)) & 1)) &
         0xFFFF;
  }
...
int main(int argc, char *argv[]) {
  ...
  case 0: {
    ...
    int benchRet = ({
      ...
      __typeof__(dlfb_lfsr16(actual_count)) __perf_ret =
          dlfb_lfsr16(actual_count);
      ...
      // Write data
      fprintf(f_csv, "dlfb_lfsr16,%lu,%lu,%lu,%lu newline", count_cycles,
