...
int main(int argc, char *argv[]) {
    ...
    if (fd_cache_miss == -1) { perror("perf_event_open (cache misses)"); exit(1); }
          ...
          __typeof__(__swapin_nr_pages(prev_offset,offset,hits,max_pages,prev_win)) __perf_ret = __swapin_nr_pages(prev_offset,offset,hits,max_pages,prev_win);
    ...
    // Write data
    fprintf(f_csv, "__swapin_nr_pages,%lu,%lu,%lu,%lu newline",
            count_cycles,
            count_instructions,
            count_cache_miss,
            count_cache_ref);
