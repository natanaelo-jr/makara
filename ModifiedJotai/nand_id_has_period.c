...
__attribute__((used)) static int nand_id_has_period(u8 *id_data, int arrlen,
                                                    int period) {
  int i, j;
  for (i = 0; i < period; i++)
...
int main(int argc, char *argv[]) {
  ...
  case 0: {
    ...
    int benchRet = ({
      ...
      __typeof__(nand_id_has_period(id_data, arrlen, period)) __perf_ret =
          nand_id_has_period(id_data, arrlen, period);
      ...
      // Write data
      fprintf(f_csv, "nand_id_has_period,%lu,%lu,%lu,%lu newline", count_cycles,
