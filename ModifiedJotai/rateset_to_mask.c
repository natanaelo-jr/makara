...
rateset_to_mask(struct ieee80211_supported_band *sband, u8 *rates,
                u8 rates_len) {
  u8 i;
  u32 mask = 0;
...
int main(int argc, char *argv[]) {
  ...
  case 0: {
    ...
    struct ieee80211_supported_band *sband =
        (struct ieee80211_supported_band *)malloc(
            _len_sband0 * sizeof(struct ieee80211_supported_band));
    for (int _i0 = 0; _i0 < _len_sband0; _i0++) {
      sband[_i0].n_bitrates = ((-2 * (next_i() % 2)) + 1) * next_i();
      int _len_sband__i0__bitrates0 = 1;
      sband[_i0].bitrates = (struct ieee80211_rate *)malloc(
          _len_sband__i0__bitrates0 * sizeof(struct ieee80211_rate));
    ...
    unsigned long *rates =
        (unsigned long *)malloc(_len_rates0 * sizeof(unsigned long));
    ...
    int benchRet = ({
      ...
      __typeof__(rateset_to_mask(sband, rates, rates_len)) __perf_ret =
          rateset_to_mask(sband, rates, rates_len);
      ...
      // Write data
      fprintf(f_csv, "rateset_to_mask,%lu,%lu,%lu,%lu newline", count_cycles,
