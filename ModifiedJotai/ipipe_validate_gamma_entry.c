...
ipipe_validate_gamma_entry(struct vpfe_ipipe_gamma_entry *table, int size) {
  ...
  for (i = 0; i < size; i++)
    if (table[i].slope > GAMMA_MASK || table[i].offset > GAMMA_MASK)
      return -EINVAL;
...
int main(int argc, char *argv[]) {
  ...
  case 0: {
    ...
    struct vpfe_ipipe_gamma_entry *table =
        (struct vpfe_ipipe_gamma_entry *)malloc(
            _len_table0 * sizeof(struct vpfe_ipipe_gamma_entry));
    ...
    int benchRet = ({
      ...
      __typeof__(ipipe_validate_gamma_entry(table, size)) __perf_ret =
          ipipe_validate_gamma_entry(table, size);
      ...
      // Write data
      fprintf(f_csv, "ipipe_validate_gamma_entry,%lu,%lu,%lu,%lu newline",
