...
__attribute__((used)) static int sge_data_len(struct ib_sge *sg_list,
                                              int num_sge) {
  int i, len = 0;
...
int main(int argc, char *argv[]) {
  ...
  case 0: {
    ...
    struct ib_sge *sg_list =
        (struct ib_sge *)malloc(_len_sg_list0 * sizeof(struct ib_sge));
    ...
    int benchRet = ({
      ...
      __typeof__(sge_data_len(sg_list, num_sge)) __perf_ret =
          sge_data_len(sg_list, num_sge);
      ...
      // Write data
      fprintf(f_csv, "sge_data_len,%lu,%lu,%lu,%lu newline", count_cycles,
