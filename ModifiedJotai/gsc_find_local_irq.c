...
int main(int argc, char *argv[]) {
    ...
    if (fd_cache_miss == -1) { perror("perf_event_open (cache misses)"); exit(1); }
          ...
          __typeof__(gsc_find_local_irq(irq,global_irqs,limit)) __perf_ret = gsc_find_local_irq(irq,global_irqs,limit);
    ...
    // Write data
    fprintf(f_csv, "gsc_find_local_irq,%lu,%lu,%lu,%lu newline",
            count_cycles,
            count_instructions,
            count_cache_miss,
            count_cache_ref);
