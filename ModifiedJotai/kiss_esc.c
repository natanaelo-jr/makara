...
__attribute__((used)) static int kiss_esc(unsigned char *s, unsigned char *d,
                                          int len) {
  unsigned char *ptr = d;
  unsigned char c;
...
int main(int argc, char *argv[]) {
  ...
  case 0: {
    ...
    int benchRet = ({
      ...
      __typeof__(kiss_esc(s, d, len)) __perf_ret = kiss_esc(s, d, len);
      ...
      // Write data
      fprintf(f_csv, "kiss_esc,%lu,%lu,%lu,%lu newline", count_cycles,
