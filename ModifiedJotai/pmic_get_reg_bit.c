...
int main(int argc, char *argv[]) {
    ...
    if (fd_cache_miss == -1) { perror("perf_event_open (cache misses)"); exit(1); }
          ...
          __typeof__(pmic_get_reg_bit(address,table,count,reg,bit)) __perf_ret = pmic_get_reg_bit(address,table,count,reg,bit);
    ...
    // Write data
    fprintf(f_csv, "pmic_get_reg_bit,%lu,%lu,%lu,%lu newline",
            count_cycles,
            count_instructions,
            count_cache_miss,
            count_cache_ref);
